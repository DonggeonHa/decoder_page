...
let guide = `# JSP 수동 반영 가이드 — Java 8 / 기존 ZXing / 최대 300장
...
MAX_QR_COUNT를 300, MAX_SOURCE_BYTES를 5000000으로 맞춥니다. 이 값은 단독 사용자의 263장 생성 사례에 맞춘 설정이며 운영 안전 최대치를 보장하지 않습니다. 나머지 값도 아래와 맞춥니다.
` + code(block('static final String RAW_PREFIX', 'static class QrPayload')) + `
한 장의 조각 길이는 1200자 그대로입니다. 디코더도 최대 300장, 해제 결과 5000000 bytes로 맞췄으므로 추후 이 두 한도를 늘릴 때는 디코더 protocol.js의 상수도 같이 변경해야 합니다.
...
` + code(buildMethods) + `
300장이 넘으면 필요한 장수도 오류에 표시됩니다. 최종 payload 길이는 헤더를 포함해 매 조각 검사합니다.
...
4. 50장이 넘던 로그를 입력합니다. 300장 이하면 생성되고, 넘으면 필요한 장수가 표시되어야 합니다.
...
300장은 항상 만드는 장수가 아니라 상한입니다. 입력의 압축 후 크기에 따라 실제 장수가 정해집니다. WAS/프록시의 POST 제한은 JSP의 500만 bytes와 별개이며 폼 URL 인코딩으로 요청 본문이 더 커질 수 있습니다. 실제 WAS 설정은 변경하지 않았습니다.
